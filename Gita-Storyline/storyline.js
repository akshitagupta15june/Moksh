...
document.addEventListener("keydown", (e) => {
  if (e.keyCode == 37) {
    currentQuestionIndex =
      (currentQuestionIndex - 1 + question.length) % question.length;
    showQuestion();
...
let isFluteAudioEnabled = true; // Flag to keep track of flute audio state
let isConversationAudioEnabled = true; // Flag to keep track of conversation audio state

function updateConversationAudioElements() {
  arjunaAudioElement.text = question[currentQuestionIndex].Arjuna;
  krishnaAudioElement.text = question[currentQuestionIndex].Krishna;
}

function toggleConversationAudio() {
  var conversationAudioToggle = document.getElementById(
    "conversation-audio-toggle"
  );

  // Cancel the current speech synthesis
  speechSynthesis.cancel();

  if (isConversationAudioEnabled) {
    // Pause conversation audio
    // No need to do anything additional, as we've already canceled the speech
  } else {
    // Start the conversation audio from the beginning
    updateConversationAudioElements();

    // Set up onend event handlers to play the next part
    arjunaAudioElement.onend = () => {
      setTimeout(() => {
        speechSynthesis.speak(krishnaAudioElement);
      }, 500); // Adjust the delay as needed
    };

    // Start playing Arjuna's part
    speechSynthesis.speak(arjunaAudioElement);
  }

  // Toggle conversation audio state
  isConversationAudioEnabled = !isConversationAudioEnabled;

  // Update the corresponding button styles/icons
  if (isConversationAudioEnabled) {
    conversationAudioToggle.innerText = "Pause";
    conversationAudioToggle.style.backgroundColor = "#DDA0DD";
  } else {
    conversationAudioToggle.innerText = "Play";
    conversationAudioToggle.style.backgroundColor = "#002D62";
  }
}
